...
DISPLAY_REWARD_THRSHOLD = 100 #当回合总reward大于400时显示模拟窗口

env = gym.make('CartPole-v0')  #CartPole这个模拟
env = env.upwrapped  # 取消限制
...
# 这里时让计算机跑完一整个回合才更新一次，之前的QLearn等在回合中每一步都可更新参数
for i_episode in range(3000):
    ...
    while True:
        ...
        RL.store_transition(observation,action,reward) #存储着一回合的transition
        if done:
            ...
            if running_reward > DISPLAY_REWARD_THRSHOLD: # 判断是否显示
                RENDER = True
